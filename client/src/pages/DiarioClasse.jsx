...
import { getTurmaById, getAlunosMatriculados, registrarAula } from '../services/sysconex';
...
export default function DiarioClasse() {
  ...
  async function carregarDados() {
    try {
      // Busca dados em paralelo usando o sysconex (API Real)
      const [dadosTurma, dadosMatriculas] = await Promise.all([
        getTurmaById(id),
        getAlunosMatriculados(id)
      ]);

      // A API retorna o objeto direto ou dentro de um array dependendo da rota antiga,
      // mas sysconex.getTurmaById retorna response.data. Vamos garantir:
      setTurma(dadosTurma);

      // getAlunosMatriculados retorna { data: [...], pagination: ... }
      const listaMatriculas = dadosMatriculas.data || [];

      // Mapeia para o formato que a tela espera
      // A rota real /turmas/:id/matriculas já retorna nome_completo e cpf no objeto
      const alunosFormatados = listaMatriculas.map(item => ({
        id: item.matricula_id, // Usamos o ID da matrícula para o diário
      ...
      console.error(error);
      alert("Erro ao carregar dados da turma ou alunos.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSalvar() {
    ...
    try {
      ...
      const payload = {
        professor_id: user.id || user.user?.id, // Garantia de pegar o ID mesmo se estrutura mudar
        data_aula: dataAula,
        conteudo: conteudo,
