...
// Agora recebemos props para controlar se tá aberto ou fechado no mobile
export function Sidebar({ isOpen, onClose }) {
  ...
  const currentMenu = user?.perfil?.id ? (menus[user.perfil.id] || []) : [];

  return (
    <>
      {/* OVERLAY (Fundo escuro quando abre no mobile) */}
      <div 
        className={clsx(
          "fixed inset-0 bg-black/50 z-40 md:hidden transition-opacity",
          isOpen ? "opacity-100 visible" : "opacity-0 invisible pointer-events-none"
        )}
        onClick={onClose} // Clica fora pra fechar
      />
      ...
      <aside 
        className={clsx(
          ...
          // No mobile: esconde (translate-x-full) se não tiver open. No Desktop (md): sempre mostra (translate-x-0)
      ...
      >
        ...
        <div className="p-6 border-b border-slate-800 bg-slate-800/50">
          ...
          <p className="font-semibold truncate">{user?.pessoa?.nome_completo?.split(' ')[0]}</p>
          ...
          {currentMenu.map((item) => {
            ...
            return (
              <Link
                key={item.path}
                to={item.path}
                onClick={onClose} // Fecha o menu ao clicar num link (no mobile)
                className={clsx(
